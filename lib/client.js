...
function Client(baseUrl, user, pass, options) {
   ...
   * @prop {number} options.requestTimeout - Integer containing the number of milliseconds to wait for a request to respond before aborting the request
   * @prop {number} options.handshakeTimeout - Timeout in milliseconds for the WebSocket handshake request
...
Client.prototype._attachApi = function () {
  ...
  return new Promise(function(resolve, reject) {
    ...
    request({ url: ariUrl, timeout: self._connection.options.requestTimeout }, function (err) {
      if (err &&
...
Client.prototype.start = function (apps, subscribeAll, callback) {
  ...
  return new Promise(function(resolve, reject) {
     ...
     */
    function connect () {
      self._ws = new WebSocket(wsUrl, null, { handshakeTimeout: self._connection.options.handshakeTimeout });
